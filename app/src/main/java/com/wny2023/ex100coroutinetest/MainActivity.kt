package com.wny2023.ex100coroutinetest
...
import com.wny2023.ex100coroutinetest.databinding.ActivityMainBinding
...
//            }
        ...
        // 실습 2) CoroutineScope 비동기 실행
        // CoroutineScope는 GlobalScope와 다르게 해당 작업을 어떤 스레드에 보낼지 결정하는
        // Dispatcher[디스패처]를 지정해야 함
        // Dispatcher의 종류
        ...
        // 4) Dispatchers.Unconfined- 조금 특별한 디스패치[ 해당 코루틴을 호출하는 스레드에서 실행]
        binding.btn2.setOnClickListener {
            //Dispatchers.Default사용
            CoroutineScope(Dispatchers.Default).launch {
                for(n in 100..110){
                    Log.d("TAG","n:$n - ${Thread.currentThread().name}")
//                    binding.tv.text ="n: $n ${Thread.currentThread().name}" //UI는 메인만가능
                    delay(500)
                }
            }
        }
        ...
        binding.btn3.setOnClickListener {
            CoroutineScope(Dispatchers.Main).launch {
                for(n in 200..210){
                    binding.tv.text="n: $n ${Thread.currentThread().name}"//MainThread이므로 UI작업가능
                    delay(500)
                }
            }
        }
        ...
        binding.btn7.setOnClickListener {
            CoroutineScope(Dispatchers.Default).launch {
                ...
                }.join() //작업1이 끝날때 가지 다른 코루틴은 실행을 대기함
